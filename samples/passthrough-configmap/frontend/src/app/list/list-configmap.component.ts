...
import { ConfigMapDemo, ConfigMapService, REMOTE_UserSession } from '../configmap.service';
...
//
// PASSTHROUGH resource: no provisioning ticket exists, so there is deliberately NO "Track Provisioning"
// action here — the object is applied synchronously on create/update.
@Component({
  selector: 'cm-list',
  ...
  template: `
    <div class="card datatable-card">
      <searchable-datatable
        ...
        <ngx-datatable-column name="Namespace" [flexGrow]="120">
          <ng-template ngx-datatable-cell-template let-row="row">{{ row.spec?.namespace || '—' }}</ng-template>
        </ngx-datatable-column>

        <ngx-datatable-column name="Keys" [flexGrow]="80">
          <ng-template ngx-datatable-cell-template let-row="row">{{ keyCount(row) }}</ng-template>
        </ngx-datatable-column>

        <ngx-datatable-column name="UID" [flexGrow]="180">
          <ng-template ngx-datatable-cell-template let-row="row">
            <span class="text-monospace font-small-3">{{ row.result?.uid || '—' }}</span>
          </ng-template>
        </ngx-datatable-column>
...
export class ListConfigMapComponent implements OnInit {
  ...
  protected keyCount(r: ConfigMapDemo): number {
    return Object.keys(r.spec?.data ?? {}).length;
  }
  ...
  protected view(r: ConfigMapDemo): void {
    this.router.navigate(['view', r.id], { relativeTo: this.route });
  }

  protected edit(r: ConfigMapDemo): void {
